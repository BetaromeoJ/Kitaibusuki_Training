...
| 今日の研修 | `#today` | 研修テーマ＋SESSION 01・02（開くボタンで全画面表示） |
| SESSION 01 全画面ビュー | `#session01-view` | 音声×生成AI（演習・プロンプト・注意事項）。メニュー等のリンクから開き、幕の演出で入場 |
| SESSION 02 全画面ビュー | `#session02-view` | Gemini Notebook×Canva（ワークフロー）。同上 |
...
## SESSION 01・02の全画面ビューについて

ヘッダーメニュー・「SESSION 01を開く」ボタン・「研修後に復習する」のクイックリンクなど、
`href="#session01"` または `href="#session02"` を持つリンクをクリックすると、通常のスクロールでは
なく、そのセッションだけを覆う全画面ビューが開きます。入場時にはサイトの青（`--color-blue-deep`）の
幕が下から上へ流れて消える演出が入ります。ビュー右上の「閉じる」ボタン、または Esc キーで元の画面に
戻れます。この動作は `assets/js/main.js` の `initSessionViews()` で制御しています。
...
- [ ] SESSION 01・02の全画面ビューが、メニューやボタンから正しく開くか。幕の演出のあとに内容が表示されるか
